...
void setup() 
{
  ...
  Domoticz dom;
  WiFiClient mqttClient(client);
  dom.InitialiseDomoticz(mqttClient);
  dom.CheckDomoticz();
  ...
  else 
  {
    ...
    String payload;
    int httpCode = pv.GetStatus(today, payload);
    SOLAR statusToday = pv.ParseStatus(payload, httpCode);

    httpCode = pv.GetStatus(yesterday, payload);
    SOLAR statusYesterday = pv.ParseStatus(payload, httpCode);

    int percentage = Delta(statusToday, statusYesterday);

    httpCode = pv.MonthAggregate(today, payload);
    SOLAR thisMonth = pv.ParseAggregate(payload, httpCode);

    httpCode = pv.MonthAggregate(last_month, payload);
    SOLAR lastMonth = pv.ParseAggregate(payload, httpCode);
    
    httpCode = pv.YearAggregate(today, payload);
    SOLAR thisYear = pv.ParseAggregate(payload, httpCode);

    httpCode = pv.YearAggregate(last_year, payload);
    SOLAR lastYear = pv.ParseAggregate(payload, httpCode);
    ...
    epdPrintUpdateTime(pv.getTimeString(today));

    float voltage = ReadBatteryVoltage();
    epdPrintBatteryState(BatteryPercentage(voltage), voltage);
    dom.LogValue2Domoticz(1509, voltage);
